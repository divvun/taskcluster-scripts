...
def create_divvun_manager_macos_task():
    return (
        ...
        .with_gha("build_service_x86",
            GithubAction("actions-rs/cargo", {
            "command": "build",
        ...
        """))
        .with_gha("build_divvun_manager", GithubActionScript("""
            /bin/bash scripts/build.sh
        """).with_env("VERSION", "${{ steps.version.outputs.version }}").with_secret_input("macos_developer_account", "divvun", "macos.developerAccount").with_secret_input("macos_notarization_app_pwd", "divvun", "macos.appPassword"))
