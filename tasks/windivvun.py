...
def create_windivvun_tasks():
    ...
    task = windows_task("Windivvun installer").with_script(
        "mkdir artifacts", as_gha=True
    )
    for (task_id, arch) in windivvun_tasks:
        (
            task.with_script(
                f"mkdir artifacts/windivvun-{arch}", as_gha=True
            ).with_curl_artifact_script(
                task_id,
                "windivvun.dll",
                out_directory=f"artifacts/windivvun-{arch}",
                as_gha=True,
            )
        )
    for (task_id, arch) in mso_tasks:
        (
            task.with_script(
                f"mkdir artifacts/divvunspell-mso-{arch}", as_gha=True
            ).with_curl_artifact_script(
                task_id,
                "divvunspellmso.dll",
                out_directory=f"artifacts/divvunspell-mso-{arch}",
                as_gha=True,
            )
        )

    return (
        task.with_gha("setup", gha_setup())
        .with_gha(
            "version",
            GithubAction(
...
def create_mso_build_tasks():
    matrix = [
        ("i686-pc-windows-msvc", "vcvars32.bat", "i686"),
        ("x86_64-pc-windows-msvc", "vcvars64.bat", "x86_64"),
    ]
    mso_tasks = []
        ...
        mso_tasks.append(
            (
                ...
                .with_gha(
                    "Set MSO CWD",
                    GithubActionScript(
                        f"echo ::set-cwd::%HOMEDRIVE%%HOMEPATH%\\%TASK_ID%\\mso-nda-resources"
                    ),
                )
                .with_gha(
                    "build",
            ...
            """
                ...
                .with_gha(
                    ...
                    GithubActionScript(
                        f"""
                mv target/{triple}/release/divvunspellmso.dll ${{HOME}}
            """
                    ),
                )
